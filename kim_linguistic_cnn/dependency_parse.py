...
def write_out_deps(input, output):
    with open(input, 'r') as inputf, open(output, 'w') as outf:
        for line in inputf:
            det = line.strip().split(' ', 1)
            outf.write(str(dependency_parse(det[1])) + "\n")
...
if __name__ == "__main__":
    ...
    write_out_deps(args.input, args.input + '.deps')
